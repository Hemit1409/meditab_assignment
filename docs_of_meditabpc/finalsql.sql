...
------(1)GetById
create or replace function Hemit_uspNewPatientsRead(
                patientid int
...
declare 
--_query varchar :='';
 _query varchar:= 'select Patients.patient_id,chart_number,firstname , lastname,middlename,Patients.sex_id,sex.sex,dob,AllergyMaster.AllergyName,PatientAllergy.Note,AllergyMaster.code,PatientAllergy.PatientAllergyId,PatientAllergy.AllergyMasterId from Patients 
...
$$
declare 
initialquery varchar(2000) := 'Select Patients.patient_id, Patients.firstname, Patients.lastname, Patients.dob, Patients.chart_number, sex.sex,AllergyMaster.AllergyName,PatientAllergy.Note,AllergyMaster.code,PatientAllergy.PatientAllergyId,PatientAllergy.AllergyMasterId
From
    Patients
    LEFT JOIN sex ON Patients.sex_id = sex.sex_id 
LEFT JOIN PatientAllergy ON Patients.patient_id = PatientAllergy.patient_id 
...
From
    Patients
    LEFT JOIN sex ON Patients.sex_id = sex.sex_id 
LEFT JOIN PatientAllergy ON Patients.patient_id = PatientAllergy.patient_id and PatientAllergy.isdeleted=false 
LEFT JOIN AllergyMaster ON PatientAllergy.AllergyMasterId = AllergyMaster.AllergyMasterId
where Patients.isdeleted=false and 1=1 ';
conditional varchar(3000) := '';
