...
-- dynamic query trial , query(4) version-2

CREATE OR REPLACE FUNCTION patient_all_data (PageNumber INTEGER = 3,
  PageSize INTEGER = 3, sortBy varchar default null, searchBy in varchar default null)
returns table(patientid integer, firstname varchar, lastname varchar, dob date, chart_number varchar, sex varchar, race varchar, country varchar, zip varchar, city varchar, _state varchar, street varchar, phone varchar) as 
$$ 
begin 
	return query execute 
	'Select t1.patient_id, t1.firstname, t1.lastname, t1.dob, t1.chart_number, t2.sex, t4.race, t5.country, t5.zip, t5.city, t5._state, t5.street, t8.phone From Patients as t1 LEFT JOIN sex as t2 ON t1.sex_id = t2.sex_id LEFT JOIN RACE as t3 ON t1.CHART_NUMBER = t3.CHART_NUMBER LEFT join raceTYPE as t4 on t1.CHART_NUMBER = t3.CHART_NUMBER and t3.race_type_id=t4.race_type_id LEFT JOIN Contact_preference as t7 on t1.CHART_NUMBER = t7.CHART_NUMBER and peference_type_id=1 LEFT join Addresses as t5 on t1.patient_id = t5.patient_id and t7.address_id = t5.address_id LEFT join phone as t8 on t8.ph_id = t7.phone_id WHERE ' || 
	searchBy ||
	'order by ' || sortBy || 'LIMIT' || PageSize ||
  'OFFSET' || '((' || PageNumber-1 || ') *' || PageSize || ')';
end;
$$ language plpgsql;

select * from patient_all_data(1,4, sortBy => 't1.lastname');
select * from patient_all_data(1,3, sortBy => 't1.lastname', searchBy => 't1.lastname = ''RANA'' ');
select * from patient_all_data(2,4, sortBy => 't2.sex',searchBy => 't1.lastname = ''RANA''and t1.firstname = ''HEMIT'' ');
select * from patient_all_data(3,2, sortBy => 't1.dob');
...
begin
	...
	if firstName is not NULL then
		 conditional := 'where t1.firstname = '''||first_Name||'''';
	end if;

	if lastName != '' then 
		if conditional != '' then
			conditional := conditional||' and t1.lastname = '''||last_Name||'''';
		else 
			conditional := 'where t1.lastname = '''||last_Name||'''';
		end if;
	end if;
	if gender != '' then 
		if conditional != '' then
			conditional := conditional||' and t2.sex = '''||gender||'''';
		else 
			conditional := 'where t2.sex = '''||gender||'''';
		end if;
	end if;
	if dateofbirth != '' then 
		if conditional != '' then
			conditional := conditional||' and t1.dob = '''||dateofbirth||'''';
		else 
			conditional := 'where t1.dob = '''||dateofbirth||'''';
		end if;
	end if;
...
select * from search_patient(1,8,first_Name=>'HEMIT',last_Name=>'RANA');
select * from search_patient(2,2,last_Name=>'RANA',gender=>'MALE');

select * from search_patient(1,2,last_Name=>'RANA',orderby=>'t1.dob');
