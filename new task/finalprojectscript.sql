...
create table Hemit_AllergyMaster(
...
INSERT INTO Hemit_AllergyMaster(Code,AllergyName) VALUES('166.63','MILK');
INSERT INTO Hemit_AllergyMaster(Code,AllergyName) VALUES('166.89','PEANUT');
INSERT INTO Hemit_AllergyMaster(Code,AllergyName) VALUES('166.23','LATEX');
select * from Hemit_AllergyMaster;

select * from Hemit_Patients;


create table Hemit_PatientAllergy(
    ...
    REFERENCES Hemit_AllergyMaster(AllergyMasterId)
);
INSERT INTO Hemit_PatientAllergy(patient_id,AllergyMasterId,Note) VALUES(3,1,'note1');
INSERT INTO Hemit_PatientAllergy(patient_id,AllergyMasterId,Note) VALUES(3,2,'note2');
INSERT INTO Hemit_PatientAllergy(patient_id,AllergyMasterId,Note) VALUES(10,3,'note3');
select * from Note;

ALTER TABLE Hemit_PatientAllergy
...
------(1)GetById
create or replace function Hemit_uspNewHemit_PatientsRead(
			...
			_orderby in varchar default 'Hemit_Patients.patient_id',
...
declare 
--_query varchar :='';
 _query varchar:= 'select Hemit_Patients.patient_id,chart_number,firstname , lastname,middlename,Hemit_Patients.sex_id,Hemit_Sex.sex,dob,Hemit_AllergyMaster.AllergyName,Hemit_PatientAllergy.Note,Hemit_AllergyMaster.code,Hemit_PatientAllergy.PatientAllergyId,Hemit_PatientAllergy.AllergyMasterId from Hemit_Patients 
left join Hemit_Sex on Hemit_Sex.sex_id=Hemit_Patients.sex_id
LEFT JOIN Hemit_PatientAllergy ON Hemit_Patients.patient_id = Hemit_PatientAllergy.patient_id 
LEFT JOIN Hemit_AllergyMaster ON Hemit_PatientAllergy.AllergyMasterId = Hemit_AllergyMaster.AllergyMasterId'; 
 
 begin
     _query := _query || ' where Hemit_Patients.isdeleted=false and Hemit_Patients.patient_id='||$3;--||;--' LIMIT 1';
     raise notice '%',_query;
...
select * from Hemit_uspNewHemit_PatientsRead(_patientid=>4);




--(2) Patient Create
CREATE or replace function Hemit_uspNewHemit_patientscreate(_PageNumber INTEGER = 1,
  			_PageSize INTEGER = 20,
			_patientid int default null,
			_FirstName varchar default '',
			_MiddleName varchar(100) default '', 
...
sexid int;
begin
	sexid= (select sex_id from Hemit_Sex where sex=_SexId);
    INSERT INTO Hemit_Patients(firstname,lastname,middlename,sex_id,dob) VALUES(_FirstName,_MiddleName,_LastName,sexid,_DOB::date) returning patient_id into primarykey; 
    return primarykey;
--   (SELECT patient_id FROM Patients ORDER BY created_on DESC LIMIT 1);  
End;
$$ language plpgsql;

select Hemit_uspNewHemit_patientscreate(_FirstName=>'RIMA',_MiddleName=>'R',_LastName=>'PATEL',_SexId=>'FEMALE',_DOB=>'1994-1-13');
select * from Hemit_Patients;

--(3) update
DROP FUNCTION hemit_uspnewhemit_patientsupdate(integer,character varying,character varying,character varying,character varying,date);

create or replace function Hemit_uspNewHemit_patientsupdate(_PageNumber INTEGER = 1,
  			_PageSize INTEGER = 20,
			_patientid int default null,
			_FirstName varchar default '',
			_MiddleName varchar(100) default '', 
...
$$
declare 
_initialquery varchar :='update Hemit_Patients set firstname = '''||$4||''',middlename='''||$5||''',lastname='''||$6||''',sex_id=(select sex_id from Hemit_Sex where sex = '''||$8||'''),dob='''||$9||''' where patient_id='||$3||' and isdeleted=false returning patient_id';
...
select * from Hemit_uspNewHemit_patientsupdate(_patientid=>11,_FirstName=>'SHRUTI',_MiddleName=>'S',_LastName=>'RANA',_SexId=>'FEMALE',_DOB=>'2002-01-01');

select * from Hemit_uspNewHemit_patientsupdate(14,'SUMEET','PATEL','S','MALE','2003-01-01');


--(4)
create or replace function Hemit_uspNewHemit_patientsdelete(
                _PageNumber INTEGER = 1,
  			_PageSize INTEGER = 20,
			_patientid int default null,
			_FirstName varchar default '',
...
declare 
--_query varchar :='';
 _query varchar:= 'update Hemit_Patients set isdeleted=true'; 
...
select * from Hemit_uspNewHemit_patientsdelete(_patientid=>3);

--(5)getListall
create  or replace function  
Hemit_uspNewHemit_patientsReadList(
			_PageNumber INTEGER = 1,
  			_pagesize INTEGER = 20,
			_patientid int default null,
			_FirstName varchar default '',
...
$$
declare 
initialquery varchar(2000) := 'Select Hemit_Patients.patient_id, Hemit_Patients.firstname, Hemit_Patients.lastname, Hemit_Patients.dob, Hemit_Patients.chart_number,Hemit_Sex.sex,COALESCE (Hemit_PatientAllergy.PatientAllergyId,0) as patientAllergyid,COALESCE(Hemit_PatientAllergy.AllergyMasterId,0) as AllergyMasterId ,COALESCE (Hemit_AllergyMaster.AllergyName,'''') as AllergyName,COALESCE (Hemit_PatientAllergy.note,'''') as note,Hemit_AllergyMaster.code,
DENSE_RANK() OVER (
		ORDER BY Hemit_Patients.patient_id
	) as denserow 
From
    Hemit_Patients
    LEFT JOIN Hemit_Sex ON Hemit_Patients.sex_id = Hemit_Sex.sex_id 
LEFT JOIN Hemit_PatientAllergy ON Hemit_Patients.patient_id = Hemit_PatientAllergy.patient_id and Hemit_PatientAllergy.isdeleted=false 
LEFT JOIN Hemit_AllergyMaster ON Hemit_PatientAllergy.AllergyMasterId = Hemit_AllergyMaster.AllergyMasterId
where Hemit_Patients.isdeleted=false and 1=1 ';
conditional varchar(3000) := '';
 
begin
	initialquery := 'select * from(' || initialquery
	|| case when $3 != 0 then ' and Hemit_Patients.patient_id = '''||$3||'''' else '' end 
	|| case when $4 != '' then ' and Hemit_Patients.firstname = '''||$4||'''' else '' end 
	||case when $6 != '' then ' and Hemit_Patients.lastname = '''||$6||'''' else '' end 
	||case when $8 != '' then ' and Hemit_Sex.sex = '''||$8||'''' else ''  end
	|| case when $9 != '' then ' and Hemit_Patients.dob = '''||$9::date||'''' else ''  end
	||case when $10 != '' then ' and Hemit_AllergyMaster.AllergyName = '''||$10||'''' else '' end
	||' ORDER BY '|| $11|| ' ' || $12
	/*||
	' OFFSET (('||$1||'-1) *'|| $2||')'*/
...
--||' LIMIT '|| $2 
 ...
 select * from Hemit_uspNewHemit_patientsReadList(_PageNumber=>1,_PageSize=>10);
 select * from Hemit_uspNewHemit_patientsReadList(_FirstName=>'Priyansh');

SELECT * FROM Hemit_uspNewHemit_PatientsReadList (_patientid => '0',_firstname => 'Priyansh', _middlename => NULL,_lastname => NULL,_sexid => NULL,_dob => NULL,_allergyname => NULL,_orderby => 'Hemit_Patients.patient_id', _ordertype => 'ASC');

SELECT * FROM Hemit_uspNewHemit_PatientsReadList(_patientid => '53',_firstname => '', _middlename => NULL,_lastname => NULL,_orderby => 'Hemit_Patients.patient_id', _sexid => NULL,_ordertype => 'ASC', _dob => NULL,_allergyname => NULL);


SELECT * FROM Hemit_uspNewHemit_PatientsReadList (_patientid => '0',_firstname => NULL,_middlename => NULL,_lastname => NULL,_orderby => 'Hemit_Patients.patient_id', _sexid => 'FEMALE', _ordertype => 'ASC', _dob => NULL,_allergyname => NULL);



Hemit_uspNewHemit_patientscreate



--(11)allergy create
CREATE or replace function Hemit_uspNewHemit_PatientAllergycreate(
_patientid INT, 
...
primarykey integer;
BEGIN
    INSERT INTO Hemit_PatientAllergy(patient_id,AllergyMasterId,Note) VALUES(_patientid,_allergymasterid,_note) returning PatientAllergyId into primarykey; 
    return primarykey;
--   (SELECT patient_id FROM Patients ORDER BY created_on DESC LIMIT 1);  
End;
$$ language plpgsql;
...
select Hemit_uspNewHemit_PatientAllergycreate(6,1,'bad');

SELECT * FROM Hemit_uspNewHemit_PatientAllergyCreate ( _patientid =>53,_allergyname => NULL,_note => 'LATEX added', _patientallergyid => NULL,_allergymasterid => '3')



CREATE or replace function Hemit_uspNewHemit_PatientAllergyupdate(
_patientid INT default null, 
...
declare 
createdkey integer;
_initialquery varchar :='update Hemit_PatientAllergy set AllergyMasterId=(select AllergyMasterId from Hemit_AllergyMaster where AllergyName = '''||$4||'''),Note='''||$3||''' where patient_id='||$1||' and PatientAllergyId='||$5||' returning PatientAllergyId';
-- _updatequery varchar:= ; 
-- _selectquery varchar:= '';
 begin
     --_initialquery := _updatequery || _selectquery ||'';
...
select * from Hemit_uspNewHemit_PatientAllergyupdate(_patientid=>4,_allergyname=>'LATEX',_note=>'S',_patientallergyid=>5);


SELECT * FROM Hemit_uspNewHemit_PatientAllergyUpdate ( _patientid =>4,_allergyname => 'LATEX',_note => 'updated2 from new again', _patientallergyid => '25',_allergymasterid => NULL)

CREATE or replace function Hemit_uspNewHemit_PatientAllergydelete(
_patientid INT default null, 
_allergymasterid INT default null, 
_note varchar(100) default null,
_allergyname varchar(100) default null,
...
deletedkey integer;
_query varchar :='';
 --_query varchar:= 'update Hemit_PatientAllergy set isdeleted=true'; 
 
 begin
     _query := 'update Hemit_PatientAllergy set isdeleted=true where PatientAllergyId='||$5||' returning PatientAllergyId';
     raise notice '%',_query;
    return query execute _query using _patientallergyid;
 end;
$$ language plpgsql;

select * from Hemit_uspNewHemit_PatientAllergydelete(_patientallergyid=>18);
...
select * from HemitDeleted_PatientanditsAllergy(patientid=>3);
/***************************************************************************************************
***************************************************************************************************
****************************************************************************************************/
...
$$
declare 
initialquery varchar(2000) := 'Select Patients.patient_id, Patients.firstname, Patients.lastname, Patients.dob, Patients.chart_number, Hemit_Sex.sex,Hemit_AllergyMaster.AllergyName,Hemit_PatientAllergy.Note,Hemit_AllergyMaster.code,Hemit_PatientAllergy.PatientAllergyId,Hemit_PatientAllergy.AllergyMasterId
From
    Patients
    LEFT JOIN Hemit_Sex ON Patients.sex_id = Hemit_Sex.sex_id 
LEFT JOIN Hemit_PatientAllergy ON Patients.patient_id = Hemit_PatientAllergy.patient_id 
LEFT JOIN Hemit_AllergyMaster ON Hemit_PatientAllergy.AllergyMasterId = AllergyMaster.AllergyMasterId
where Patients.isdeleted=false and 1=1';
conditional varchar(3000) := '';
...
begin
	...
	||case when $5 != '' then ' and Hemit_Sex.sex = '''||$5||'''' else '' end
	||case when $6 != '' then ' and Patients.dob = '''||$6::date||'''' else '' end
	||case when $7 != '' then ' and Hemit_AllergyMaster.AllergyName = '''||$7||'''' else '' end
	||' GROUP BY Patients.patient_id,Hemit_Sex.sex ORDER BY '|| $8|| ' ' || $9||' LIMIT '|| $2 ||' OFFSET (('||$1||'-1) *'|| $2||')';
...
$$
declare 
initialquery varchar(2000) := 'select patient_id,Hemit_PatientAllergy.PatientAllergyId,Hemit_PatientAllergy.AllergyMasterId,Hemit_PatientAllergy.Note,Hemit_AllergyMaster.AllergyName,Hemit_AllergyMaster.code  
from Hemit_PatientAllergy 
LEFT JOIN Hemit_AllergyMaster ON Hemit_PatientAllergy.AllergyMasterId = Hemit_AllergyMaster.AllergyMasterId where Hemit_PatientAllergy.isdeleted=false and patient_id='||$1;
begin
	raise notice 'sql %' , initialquery;
...
$$
declare 
_initialquery varchar :='update PatientAllergy set AllergyMasterId=(select AllergyMasterId from Hemit_AllergyMaster where AllergyName = '''||$3||'''),Note='''||$4||''' where patient_id='||$2||' and PatientAllergyId='||$1||' returning PatientAllergyId';
-- _updatequery varchar:= ; 
-- _selectquery varchar:= '';
 begin
     --_initialquery := _updatequery || _selectquery ||'';
...
declare
    _initialquery varchar :='update PatientAllergy set '
	   || case when $3 != '' then 'AllergyMasterId=(select AllergyMasterId from Hemit_AllergyMaster where AllergyName = '''||$3||''')' else 'AllergyMasterId=AllergyMasterId' end
...
INSERT INTO Hemit_Sex(Sex) VALUES('MALE');
INSERT INTO Hemit_Sex(sex) VALUES('FEMALE');
INSERT INTO Hemit_Sex(sex) VALUES('UNKNOWN');
select * from Hemit_Sex;
...
--	CHART_NUMBER VARCHAR(40) NOT null unique DEFAULT to_char(nextval('PATIENT_ID'), '"CHART"0000FM'), 
    ...
    CONSTRAINT fk_sex  
    FOREIGN KEY(sex_id)   
    REFERENCES Hemit_Sex(sex_id)
);
INSERT INTO Hemit_Patients(firstname,lastname,middlename,sex_id,dob) VALUES('HEMIT','RANA','S',1,'2000-01-01');
INSERT INTO Hemit_Patients(firstname,lastname,middlename,sex_id,dob) VALUES('KHUSHI','RANA','S',2,'2005-01-01');
INSERT INTO Hemit_Patients(firstname,lastname,middlename,sex_id,dob) VALUES('KHUSHI','RANA','S',2,'2005-01-01');
select * from Hemit_Patients;

ALTER TABLE Hemit_Patients
...
From
    ...
    LEFT JOIN Hemit_Sex as t2 ON t1.sex_id = t2.sex_id
